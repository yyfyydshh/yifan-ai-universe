...
## v25 抽象空间与内容层级（2026-09-07）

本节依据用户最新反馈“这是一种概念，不能完全变星球”更新视觉方向；与下方历史记录冲突时，以本节为准。

- 首页继续采用人物中心单轴轨道，当前实际为五个项目；作品页保留七个项目的三维交互。
- 视觉为 Abstract Capability Constellation：暖白身份文字、深空底色、克制紫色焦点，用空间关系表达个人能力系统。用户最新反馈要求恢复原版对比度：项目采用原版深色渐变底与清晰亮字，禁止用透明渐隐或整体降透明度使文字发灰。
- 不使用写实地表、实体星球、土星环或按项目分配的多彩材质。用原版抽象圆形底色、弧线与轻量图标暗示空间。
- 项目名称、类别和进入提示默认可见。近大远小与遮挡形成空间感，项目文字保持不透明；远景不通过灰化文字表达。
- 背景采用固定、非网格星点；轨道使用前亮后暗的银灰线。移除无来源的双光点和大面积紫色发光按钮。
- 人物默认有可辨轮廓，交互时显现。人物层位于前后半圈之间；用亮度控制显隐，不降低整体透明度，确保后方项目被人物轮廓真实遮挡。整个前半圈按深度经过人物前方，鼠标悬停不改变前后关系；键盘聚焦可临时提升项目以保证可访问性。保留原始人物 PNG；移动端采用轻量图标与开放式项目行。
- 桌面可手动暂停／继续巡航；减少动态效果时显示静态布局。轨道路径只在尺寸变化时重建。
- 人物触发前后需要显著区分：默认亮度 0.1，移入区域时显现图层透明度 1、亮度 0.82，悬停项目时显现强度 0.85，移出后退回暗轮廓；保持遮挡基础图层不透明。减少动态偏好保持静态。
- 背景鼠标交互仅用于首页和 `/work` 项目宇宙：固定星图在鼠标附近约 200px 内渐显，停住时保留，移开后约 1 秒淡出；星尘视差不超过 5px。首页定位文字区降低背景强度，装饰不截获鼠标事件。手机、粗指针与减少动态偏好保留静态背景，其他路由不挂载交互组件。
- 用户要求加强反馈后：显现范围上限 235px，细线和星点亮度增强；鼠标真实路径形成约 460ms 的短流星尾迹。细十字星光标在链接上增加圆环提示，按下收缩，离开交互区域恢复系统光标。滚动使远景星尘与少量近景微粒以不同幅度偏移；保持原生滚动，不阻止页面滚轮或项目拖动。
- 姓名保持中文衬线，其他标题与信息保持中文无衬线；白色主行动搭配轻量文字次行动。
- 新视觉样式集中在 `app/spatial.css`，在原有样式后引入；新色值继续由 `tokens.css` 管理。项目详情、文章、职业事实和原有交互工作台继续沿用既有设计系统。
